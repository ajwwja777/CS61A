...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        count = 0
        place = nearest_place(self.place)
        if self.max_range != float('inf') and count > self.max_range:
            return None
        if self.min_range != 0 and count < self.min_range:
            count += 1
            while place:
                place = nearest_place(place.entrance)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for bee in place.bees.copy():
            Insect.reduce_armor(bee, amount+damage)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        Ant.__init__(self, armor)
        self.digesting = 0 #turns left to digest
    ...
    def action(self, gamestate):
        """to check if it is digesting.
        if so, decrement digesting.
        otherwise, eat a random bee.
        """
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.digesting <= 0 and rANTdom_else_none(self.place.bees) is not None:
