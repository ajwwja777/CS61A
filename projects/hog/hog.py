...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    ones1_d = player_score % 10
    ones2_d = opponent_score % 10
    tens2_d = (opponent_score // 10) % 10
    if abs(ones1_d - ones2_d) == tens2_d:
        return True
    else:
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    """
    不知道为什么，会遇到超时错误
    有score0, score1 = my_score, opponent_score时，会超时
    而score0, score1 = opponent_score, my_score，却不会
    """
    """
    while True:
        if who == 0:
            my_score, opponent_score = score0, score1
        else:
            my_score, opponent_score = score1, score0
        my_score += take_turn(strategy0(my_score, opponent_score), opponent_score, dice)
        if is_swap(my_score, opponent_score):
            my_score, opponent_score = opponent_score, my_score
        if my_score >= goal or opponent_score >= goal:
            break
        who = other(who)
    if who == 1:
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            this_scored0 = take_turn(num_rolls, score1, dice)
            score0 += this_scored0
            if feral_hogs and abs(num_rolls - last_scored0) == 2:
                score0 += 3
            if is_swap(score0, score1):
                score0, score1 = score1, score0
            last_scored0 = this_scored0
        else:
            num_rolls = strategy1(score1, score0)
            this_scored1 = take_turn(num_rolls, score0, dice)
            score1 += this_scored1
            if feral_hogs and abs(num_rolls - last_scored1) == 2:
                score1 += 3
            if is_swap(score1, score0):
                score0, score1 = score1, score0
            last_scored1 = this_scored1
        who = other(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(s0, s1):
        ...
        if scored > rh:
            rh = scored
            print(rh, "point(s)! That's the biggest gain yet for Player", who)
        return announce_highest(who, score, rh)
    return say
    """
    def say(*score):
        assert len(score) == 2, 'The say argument should indicate two score.'
        scored = score[who] - last_score
        rh = running_high
        if scored > rh:
            rh = scored
            print(rh, "point(s)! That's the biggest gain yet for Player", who)
        return announce_highest(who, score[who], rh)
    return say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def return_averaged(*args):
        sum = 0
        for i in range(trials_count):
            sum += original_function(*args)
        average = sum / trials_count
        return average
    return return_averaged
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    """use roll_dice, make_averaged"""
    average_score = [make_averaged(roll_dice)(num, dice) for num in range(1,11)]
    return average_score.index(max(average_score)) + 1
    # END PROBLEM 9
...
def run_experiments():
    ...
    if True:  # Change to True to test always_roll(2)
