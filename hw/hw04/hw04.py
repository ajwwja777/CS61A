...
def balanced(m):
    ...
    if is_planet(m):
        ...
        len_l, len_r = length(left(m)), length(right(m))
        return len_l*total_weight(l) == len_r*total_weight(r) and balanced(l) and balanced(r)
...
def quadratic(x, a, b, c):
    ...
    x2 = upper_bound(x)
    x_e = - b / (2*a)
    y_e = f(x_e) if x1 < x_e and x_e < x2 else f(x1)
    return interval(min(f(x1), f(x2), y_e), max(f(x1), f(x2), y_e))
