...
def composer(func=lambda x: x):
    ...
    def func_adder(g):
        ...
        nonlocal last 
        last = compose(last, g)
        # last = lambda x: last(g(x))
        return last, lambda f: func_adder(f)
    return func, func_adder
...
def g_iter(n):
    ...
    if n <= 3:
        ...
        for i in range(4, n + 1):
            res = [res[1], res[2], res[2] + res[1] * 2 + res[0] * 3]
        return res[2]
...
def missing_digits(n):
    ...
    if num == -1:
        num = 0
    return missing_digits(n // 10) + num
...
def count_change(total):
    ...
    return helper(total, find_bin(total))

def find_bin(n):
    bin1 = 1
    bin2 = 2
    while (bin1 <= n and n < bin2) == False:
        bin1 = bin1 * 2
        bin2 = bin2 * 2
    return bin1
...
def make_anonymous_factorial():
    ...
    """
    "*** CODE FROM FLYING PIG ***"
    from functools import reduce
    return lambda n:reduce(mul, range(1, n + 1))
    """
fact = lambda n: 1 if n == 1 else mul(n, fact(sub(n, 1)))
#等价于
def make_factorial_function():
    def factorial_recursive(f, k):
        return k if k == 1 else k * f(f, k - 1)
    return lambda k: factorial_recursive(factorial_recursive, k)
