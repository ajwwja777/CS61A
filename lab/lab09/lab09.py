...
def cumulative_mul(t):
    ...
    def helper(t):
        all_num = [t.label]
        for b in t.branches:
            all_num.append(helper(b))
        t.label = reduce(lambda x, y: x * y, all_num)
        return t.label
    helper(t)
...
def has_cycle_constant(link):
    ...
    "*** YOUR CODE HERE ***"
    slow = fast = link
    while slow != Link.empty and fast.rest != Link.empty:
        slow = slow.rest
        fast = fast.rest.rest
        if slow == fast:
            return True
...
def reverse_other(t):
    ...
    def helper(t, depth):
        ...
        if depth % 2 == 1:
            for b, new_label in zip(t.branches, reversed(label_lst)):
                b.label = new_label
